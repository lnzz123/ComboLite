...
class StaticReceiver : IPluginReceiver {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val message = when (action) {
            ...
            Intent.ACTION_AIRPLANE_MODE_CHANGED -> if (intent.getBooleanExtra("state", false)) "飞行模式已开启" else "飞行模式已关闭"
            Intent.ACTION_HEADSET_PLUG -> if (intent.getIntExtra("state", 0) == 1) "耳机已插入" else "耳机已拔出"
