...
abstract class ConvertAarToApkTask : DefaultTask() {
    ...
    private fun compileJavaSourcesToDex(extractDir: File, buildDir: File, sourceDir: File) {
        ...
        if (libsDir.exists() && libsDir.isDirectory) {
            libsDir.listFiles { file -> file.isFile && file.extension.equals("jar", ignoreCase = true) }?.forEach { jarFiles.add(it) }
        }

        extractDir.listFiles { file -> file.isFile && file.name.matches(Regex("classes\\d*\\.jar")) }?.forEach { if (!jarFiles.contains(it)) jarFiles.add(it) }
    ...
    private fun compileRJavaToClasses(sourceDir: File, buildDir: File) {
        ...
        val classesDir = File(buildDir, "classes"); classesDir.mkdirs()
        val androidJar = File(androidSdkPath.get(), "platforms/${androidPlatform.get()}/android.jar")
        ...
        try {
            executeShellCommand(listOf("javac", "-cp", androidJar.absolutePath, "-d", classesDir.absolutePath, "@${javaFilesList.absolutePath}"), buildDir)
            val rClassesJar = File(buildDir, "r_classes.jar")
            executeShellCommand(listOf("jar", "cf", rClassesJar.absolutePath, "-C", classesDir.absolutePath, "."), buildDir)
    ...
    private fun convertJavaContentsToDex(jarFiles: List<File>, buildDir: File, includeRClasses: Boolean) {
        ...
        command.add("--min-api"); command.add("21")
    ...
    private fun addNativeLibrariesToApk(apkFile: File, extractDir: File, buildDir: File) {
        ...
        val tempLibDir = File(buildDir, "temp_lib/lib"); tempLibDir.mkdirs()
    ...
    private fun addAssetsToApk(apkFile: File, extractDir: File, buildDir: File) {
        ...
        if (!assetsDir.exists() || !assetsDir.isDirectory || assetsDir.walkTopDown().filter { it.isFile }.toList().isEmpty()) {
    ...
    private fun buildUnsignedApk(buildDir: File, manifestFile: File) {
        ...
        val command = mutableListOf(
            ...
            "-I", androidJar.absolutePath,
            "--manifest", manifestFile.absolutePath,
            "--auto-add-overlay", "--no-version-vectors", "--no-resource-removal",
            "--emit-ids", rTxtFile.absolutePath,
            "--package-id", pluginPackageId
        )
        ...
        if (compiledResDir.exists() && compiledResDir.list()?.isNotEmpty() == true) {
            ...
            if (flatFiles.isNotEmpty()) {
                logMessage("添加${flatFiles.size}个编译后的资源")
                flatFiles.forEach { file ->
                    command.add("-R"); command.add(file.absolutePath)
                }
            }
        }
...
pluginModules.forEach { modulePath ->
    ...
    // --- 创建Debug任务 ---
    tasks.register<ConvertAarToApkTask>("convert_${baseTaskName}_debug") {
        group = "plugin debug"
        description = "自动构建 ${subproject.name} 模块并转换为Debug插件APK"

        val aarPathString = "${subproject.layout.buildDirectory.get()}/outputs/aar/${subproject.name}-debug.aar"
    ...
    // --- 创建Release任务 ---
    tasks.register<ConvertAarToApkTask>("convert_${baseTaskName}_release") {
        group = "plugin release"
        description = "自动构建 ${subproject.name} 模块并转换为Release插件APK"

        val aarPathString = "${subproject.layout.buildDirectory.get()}/outputs/aar/${subproject.name}-release.aar"
        this.aarFilePath.set(aarPathString)
...
tasks.register("cleanAllPluginOutputs") {
    ...
    doLast {
        project.layout.buildDirectory.dir("output/plugin").get().asFile.deleteRecursively()
        File(project.layout.buildDirectory.get().asFile, "logs").deleteRecursively()
        File(project.layout.buildDirectory.get().asFile, "tmp").listFiles()?.forEach { file ->
